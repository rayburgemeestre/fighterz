...
static struct GOURAUD_RANGE button_gray_range, scrollbar_gray_range, menu_gray_range, window_blue_range,
    slider_gray_range;
...
static void rectgouraud(BITMAP *bmp, int x1, int y1, int x2, int y2, GOURAUD_RANGE *range, int vert) {
  iRGB gamma;
  int x, y;
...
static void photon_textout(BITMAP *bmp, AL_CONST char *text, int x, int y, int flags, int centre) {
  int rtm, height, len;

  rtm = text_mode(-1);

  if (flags & D_DISABLED) gui_textout(bmp, text, x + 1, y + 1, white, centre);
...
static void photon_scrollbar(BITMAP *bmp, int x, int y, int w, int h, int vert, int pos, int len) {
  int slx, sly, slw, slh;
  ...
  if (vert) {
    ...
    if (sly + slh + 1 < y + h) hline(bmp, slx, sly + slh + 1, slx + slw - 1, scrollbar_gray2);
  } else {
    slx = x + pos;
    slw = len;
    sly = y;
    ...
    if (slx + slw + 1 < x + w) vline(bmp, slx + slw + 1, sly, sly + slh - 1, scrollbar_gray2);
  }
...
int d_aphoton_box_proc(int msg, DIALOG *d, int c) {
  ...
  if (msg == MSG_DRAW) photon_box(screen, d->x, d->y, d->w, d->h, normal);
...
int d_aphoton_button_proc(int msg, DIALOG *d, int c) {
  ...
  if (msg == MSG_DRAW) {
    ...
    if (d->dp)
      photon_textout(screen,
                     (char *)d->dp,
                     d->x + d->w / 2 + shift,
                     d->y + d->h / 2 - text_height(font) / 2 + shift,
                     d->flags,
                     TRUE);
...
int d_aphoton_check_proc(int msg, DIALOG *d, int c) {
  if (msg == MSG_DRAW) {
    ...
    if (d->dp)
      photon_textout(screen, (char *)d->dp, d->x + 20, d->y + d->h / 2 - text_height(font) / 2, d->flags, FALSE);
...
static AL_CONST unsigned char radio[] = {
    ...
    };

static AL_CONST unsigned char radio_sel[] = {
    /* 14 x 14 */
    };
...
int d_aphoton_radio_proc(int msg, DIALOG *d, int c) {
  if (msg == MSG_DRAW) {
    ...
    if (d->flags & D_SELECTED)
      blit(radio_sel_bmp, screen, 0, 0, d->x + 3, d->y + d->h / 2 - 7, radio_sel_bmp->w, radio_sel_bmp->h);
    else
      blit(radio_bmp, screen, 0, 0, d->x + 3, d->y + d->h / 2 - 7, radio_bmp->w, radio_bmp->h);
...
int d_aphoton_edit_proc(int msg, DIALOG *d, int c) {
  if (msg == MSG_DRAW) {
    ...
    for (x = 4; p <= b; p++) {
      ...
      text_mode(rtm);
      if (f) vline(screen, d->x + x - 1, d->y + 3, d->y + fonth + 5, black);
      if ((x += w) + w > d->w - 4) break;
    }

    if (d->flags & D_GOTFOCUS) hline(screen, d->x + 4, d->y + fonth + 4, d->x + d->w - 5, highlight);
...
static void photon_draw_scrollable_frame(DIALOG *d, int listsize, int offset, int height) {
  int pos, len;
  ...
  if (msg == MSG_DRAW) {
    ...
    for (i = 0; i < height; i++) {
      if (d->d2 + i < listsize) {
        ...
        if (x <= d->x + w) rectfill(screen, x, y, d->x + w, y + char_height - 1, bg);
      } else {
        rectfill(screen, d->x + 2, d->y + 2 + i * char_height, d->x + w, d->y + 1 + (i + 1) * char_height, list_white);
      }
    }

    if (d->y + 2 + i * char_height <= d->y + d->h - 3)
      rectfill(screen, d->x + 2, d->y + 2 + i * char_height, d->x + w, d->y + d->h - 3, list_white);
...
int d_aphoton_text_list_proc(int msg, DIALOG *d, int c) {
  if (msg == MSG_DRAW) return d_aphoton_list_proc(msg, d, c);
...
int d_aphoton_slider_proc(int msg, DIALOG *d, int c) {
  if (msg == MSG_DRAW) {
    ...
    if (vert) {
      ...
      /* marks */
      for (i = 0; i < inmark; i++) photon_slider_mark(screen, d->x + d->w - 3, d->y + 1 + i * ismark);
      ...
      /* marks */
      for (i = 0; i < inmark; i++) photon_slider_mark(screen, d->x + 1 + i * ismark, d->y + d->h - 3);
...
static AL_CONST unsigned char menu_arrow[] = {
    /* 6 x 8 */
    0x00, 0x00, 0x33, 0x00, 0x00, 0x00, 0x00, 0x00, 0x33, 0x33, 0x00, 0x00, 0x33, 0x33, 0x33, 0x33,
    0x33, 0x00, 0x33, 0x33, 0x33, 0x33, 0x33, 0x33, 0x33, 0x33, 0x33, 0x33, 0x33, 0x33, 0x33, 0x33,
    0x33, 0x33, 0x33, 0x00, 0x00, 0x00, 0x33, 0x33, 0x00, 0x00, 0x00, 0x00, 0x33, 0x00, 0x00, 0x00};
...
static void photon_draw_menu_item(MENU *m, int x, int y, int w, int h, int bar, int sel) {
  ...
  if (ugetc(m->text)) {
    ...
    if ((m->child) && (!bar)) draw_sprite(screen, menu_arrow_bmp, x + w - 12, y + (h - menu_arrow_bmp->h - 1) / 2);
  } else {
    height = text_height(font);
...
#define MAKECOL_RANGE_FROM(range) makecol(range.from.r, range.from.g, range.from.b)

#define MAKECOL_RANGE_TO(range) makecol(range.to.r, range.to.g, range.to.b)
...
static struct agup_theme the_theme = {&aphoton_fg_color,
                                      &aphoton_bg_color,
                                      aphoton_init,
                                      aphoton_shutdown,
                                      d_aphoton_box_proc,
