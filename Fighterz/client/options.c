...
#define FLICKER_DELAY 100
  ...
  while (1) {
    ...
    for (i = 0; i < 3; i++) {
      int diff = (ourtime - option[i].time), tmp = 1, times = 0, cnt;
      ...
      if (options_activeitem == i) {
        ...
        if ((option[i].flicker == 0) && (option[i].r == 0 && option[i].g == 128)) {
          ...
          textprintf(
              tmpscreen, (FONT *)dat_base[ARCADE].dat, option[i].x, option[i].y, makecol(0, 0, 0), "%s", option[i].str);
        } else {
          ...
          textprintf(tmpscreen,
                     ...
                     option[i].x,
                     option[i].y,
                     makecol(option[i].r, option[i].g, option[i].b),
                     "%s",
                     option[i].str);
        }
