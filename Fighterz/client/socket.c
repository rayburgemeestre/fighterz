...
int socket_connect() {
  ...
  if (lpHostEntry == NULL) RETURN_ERROR("Error at gethostbyname()");
  ...
  if (sock_to_serv == INVALID_SOCKET) RETURN_ERROR("Error at socket()");
  ...
  if (setsockopt(sock_to_serv, IPPROTO_TCP, TCP_NODELAY, (char *)&just_say_no, sizeof just_say_no) == -1) {
  ...
  if (nRet == SOCKET_ERROR) RETURN_ERROR("Error at connect()");
...
void socket_read() {
  ...
  if (one > 0) {
    ...
    if (r == SOCKET_ERROR) die("Error on recv()");
    ...
    while (len > 0) {
      ...
      if (!get_u16(&plen, &p2, &xlen)) break;  /* Insufficient data (short header) */
      if (xlen < plen) break;                  /* Insufficient data (short type/data) */
      if (!get_u16(&xtype, &p2, &xlen)) break; /* Impossible */
...
int socket_connect() {
  ...
  // TODO: check socke
  if (sock_to_serv == -1) RETURN_ERROR("Error at socket()");
  ...
  if (nRet == -1) RETURN_ERROR("Error at connect()");
...
void dopacket(int xtype, unsigned short len, char *dta) {
  ...
  switch (xtype) {
    ...
    case SMSG_FLAGCAPT: {
      ...
      addtext(
          "flag_capturer id=%lu,code=%lu,points red=%lu,blue=%lu", capturer_id, code, red_team_score, blue_team_score);
...
void m_flagcapturer(unsigned int capturer_id,
                    unsigned int code,
                    unsigned int red_team_score,
                    unsigned int blue_team_score) {
...
void m_background(int df_id, int pos_x, int pos_y, char *datfile) {
  ...
  if (strlen(datfile) > 128) datfile[127] = '\0';
...
void m_quit(unsigned int id, char *quit_msg) {
  ...
  // Preconditions
  if (current == NULL) die("m_quit(): current cannot be NULL");
...
void m_newuser(int id,
  ...
  if (!(new_node = (PLAYER)malloc(sizeof(user)))) die("Error allocating memory");
...
void m_kill(unsigned int victimid, unsigned int evilid, char *killstr) {
  ...
  if (evil_node != NULL && victim_node != NULL) {
    ...
    evil_node->kills++;
    addtext("*** %s(%d) fragged %s(%d)", evil_node->nick, evil_node->id, victim_node->nick, victim_node->id);
...
void m_newbullet(unsigned int id, unsigned int ownerid, double x, double y, double deg) {
  ...
  if (ownerid != our_id) play_sample((SAMPLE *)dat_sound[SHOOT].dat, 50, 128, 1000, 0);
...
void m_spawn(unsigned int id,
  ...
  // Preconditions
  if (!node) die("Invalid id for node in m_spawn");
...
void m_hit() {
  if (our_node->power > 0) our_node->power--;
}
...
void m_accel(unsigned int id, double x, double y, signed char accel, double speed) {
  ...
  // Preconditions
  if (!current) die("current is null in m_accel");
