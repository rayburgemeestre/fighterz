...
void printff_direct(char *pattern, ...) {
  ...
  if (game_started == 1 || our_spawnstatus == 1) return;
...
void parse_input() {
  ...
  if (our_node->dead == 0 || our_node->invincible == 1) {
    ...
    if ((key[KEY_LCONTROL] || key[KEY_Z]) && our_node->invincible != 1 && our_node->dead != 2) { /* Requesting fire */
...
#define TWIST 4
    ...
    if (key[KEY_UP] || key[KEY_DOWN]) {
      if (key[KEY_UP]) {
        ...
        if (ret != 1 || DONT_LOSE_VELOCITY_AT_COLLISION) {
          ...
          if (our_node->velocity == -1)
            our_node->velocity = 0;
          else if (our_node->velocity == 0)
            our_node->velocity = 1;

          /* notify server */
          if (oldvel != our_node->velocity) send_accel(5);

        } else {
          our_node->velocity = 0;
          if (oldvel != our_node->velocity) send_accel(6);
        }
      }
      if (key[KEY_DOWN] && !key[KEY_UP]) {
        ...
        if (ret != 1 || DONT_LOSE_VELOCITY_AT_COLLISION) {
          ...
          /* notify server */
          if (oldvel != our_node->velocity) send_accel(1);

        } else {
          our_node->velocity = 0;
          if (oldvel != our_node->velocity) send_accel(2);
        }
      }
    } else {
  ...
  while (keypressed()) {
    ...
    if (talk_mode == 1) {
      ...
      if (k == KEY_P) {
        char nam[80];
        sprintf(nam, "screen%lu.pcx", ourtime);
        PALETTE pal;
        get_pallete(pal);
      ...
      if (k == KEY_F11 /*&& false*/) {
        if (mod_bounce == 1) {
          ...
          our_bullet_autofiredelay = 150;
          // local test (temp):
          if (our_node->speed < our_node->max_speed) our_node->speed = our_node->max_speed;
        } else {
...
void printulist() {
  ...
  for (current = head; current; current = current->next) {
    if (current->team != 2) continue;

    if (current->bullet != 1) printul(current);
  }
...
void printul(struct data *node) {
  ...
  if (node->team == 2) {
    ...
    if (node->invincible == 2)
      color = makecol(255, 122, 119);  // invincible
    else if (node->invincible == 1)
      color = makecol(255, 65, 60);  // respawned
    else if (node->dead == 1)
      color = makecol(144, 144, 144);  // dead
    else if (node->dead == 3)
      color = makecol(183, 0, 0);  // spectate
    else
      color = makecol(255, 64, 64);  // normal (alive)
  }
  ...
  if (scoreboard_current_y == 0) {
    ...
    alfont_textout_right_aa_ex(
        bmp_scoreboard, lcdfont, buf, scoreboard_w - scoreboard_padding_right, scoreboard_current_y, color, -1);
  }
...
void debug() {
  ...
  {
    fputs(
        "Clients time:   | Addr. of node:  | Id: | Nickname:\n"
        "----------------+-----------------+-----+----------\n",
        debugfile);
    for (current = head; current; current = current->next)
      if (current->bullet != 1) {
...
void show_graphics() {
  ...
  if (save_frames_to_bitmap == 1) {
    char nam[80];
    sprintf(nam, "D:\\fighterz\\%lu.pcx", ourtime);
    PALETTE pal;
