                                                                ...
                                                                called in mainloop() at UP/DOWN stuff */
{
  ...
  int deg_diff_max = 50, demp_min = 10, demp_max = 25, rdemp;

  current2 = head;
  while (current2 != NULL) {
    if (current2->id == id) break;
...
#define BOUNCE_POSSIBLE_OR_RESTORE              \
  ...
  if (!(px > map_blocks_x) && !(px < 0) && !(py > map_blocks_y) && !(py < 0)) {
    ...
    if ((py - 1) >= 0) {
      ...
      if (field[qy][px] == '1') {
        ...
        if (cool)
          rectfill(
              bmp_shipfield, (px * blocksize), (qy * blocksize), ((px + 1) * blocksize), ((qy + 1) * blocksize), clr);
    ...
    if ((py + 1) <= map_blocks_y) {
      ...
      if (field[qy][px] == '1') {
        ...
        if (!nobounce && dabs(current2->speed) > 0.05 && AFKAATSEN)  // static minimal speed required
        {
          ...
          if (current2->deg >= (270 - deg_diff_max) && current2->deg <= 270) {
            current2->deg = 270 + ((270 - current2->deg) / rdemp);
            BOUNCE_POSSIBLE_OR_RESTORE
          }
          ...
          else if (current2->deg >= (90 - deg_diff_max) && current2->deg <= 90) {
    ...
    if ((px + 1) <= (map_blocks_x)) {
      ...
      if (field[py][qx] == '1') {
        if (cool)
          rectfill(
              bmp_shipfield, (qx * blocksize), (py * blocksize), ((qx + 1) * blocksize), ((py + 1) * blocksize), clr);
    ...
    if ((px - 1) >= 0) {
      ...
      if (field[py][qx] == '1') {                             // TODO; find out why :)
        ...
        if (!nobounce && dabs(current2->speed) > 0.05 && AFKAATSEN)  // static minimal speed required
        {
          ...
          if (current2->deg >= (360 - deg_diff_max) && current2->deg <= 360) {
            current2->deg = (360 - current2->deg) / rdemp;
          ...
          else if (current2->deg >= (180 - deg_diff_max) && current2->deg <= (180 + deg_diff_max)) {
...
#ifdef OLD_CODE
  if (collided == 1) {
    if (current2->id == our_id) { /* our ship */
...
void process_bounce(struct data *ptr, int *retval, double speed) {
  ...
  sprintf(tmp, "%d %lu COLLITION AT DEG: %f X: %f Y: %f", ptr->move, ourtime, ptr->deg, ptr->x, ptr->y);
  ...
  if (c && b) {
    ...
    *retval = 1;
    ...
    if (py > 0 && field[py - 1][px] == '1') ptr->deg += 90;  // 1
    ...
    *retval = 1;
    ...
    if (px > 0 && field[py][px - 1] == '1') ptr->deg += 90;  // 1
    ...
    *retval = 1;
    ...
    if (px < map_blocks_x && field[py][px + 1] == '1') ptr->deg -= 90;  // 1
    if (py < map_blocks_y && field[py + 1][px] == '1')
      ptr->deg -= 270;  // 2
    else
      ptr->deg -= 180;  // 3
           ...
           (c && d) )
  {
    ...
    if (ptr->deg < 0) ptr->deg += 360;
    if (ptr->deg > 360) ptr->deg -= 360;
...
int collidecheck2b(struct data *ptr) {
  ...
  current2 = head;
  while (current2 != NULL) {
    if ((struct data *)current2 == ptr) break;
    current2 = current2->next;
  }
...
int bs_collidecheck(struct data *ptr) {
  ...
  if (tmp != NULL) {
    if (tmp->bullet == 1) {
      ...
      if ((ourtime - tmp->hit_t) >= 200) tmp->impact = 0;
      ...
      while (tmp2 != NULL) {
        i = 0;
        if ((tmp2->bullet != 1) && (tmp->owner != tmp2) && (tmp2->dead == 0) && (tmp2->invincible == 0)) {
          /* Compare coordinates with this ship */
          if ((dabs(tmp2->x - (tmp->x)) <= (blocksize / 4)) && (dabs(tmp2->y - (tmp->y)) <= (blocksize / 4))) {
