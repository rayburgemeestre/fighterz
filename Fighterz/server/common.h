...
#define DEBUG2                                                                         \
  1 /*    1= SHOW FIELD(console msgs are logged to a file)                             \
                              0= CONSOLE ONLY                                          \
                                                                                       \
...
#define SetDeadSend(obj)           \
  do {                             \
    obj->enabled = STATE_DEADSEND; \
  } while (0);
#define SetDead(obj)           \
  do {                         \
    obj->enabled = STATE_DEAD; \
  } while (0);
#define SetSynched(obj)           \
  do {                            \
    obj->enabled = STATE_SYNCHED; \
  } while (0);
#define SetLag(obj)           \
  do {                        \
    obj->enabled = STATE_LAG; \
  } while (0);
#define SetGotField(obj)     \
  do {                       \
    obj->enabled = STATE_OK; \
  } while (0);
#define SetVersion(obj)              \
  do {                               \
    obj->enabled = STATE_GOTVERSION; \
  } while (0);
#define SetNew(obj)           \
  do {                        \
    obj->enabled = STATE_NEW; \
  } while (0);
