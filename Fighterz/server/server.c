...
unsigned int
    BULLET_MAX;  /**< User can only have BULLET_MAX bullets at a time _in_ the field (should be in sync with clients) */
...
void flagcheck() {
  ...
  for (lnk = head; lnk; lnk = lnk->next) {
    if (lnk->bot == 1 || lnk->bullet == 1) continue;

    // er staat iemand op de rode vlag
    if ((sqrt((lnk->x - red_x) * (lnk->x - red_x) + (lnk->y - red_y) * (lnk->y - red_y)) <= (BLOCKSIZE / 2))) {
      printf_("ok er staat iemand op de rode vlag\n");
      if ((id_has_redflag == -1) && (lnk->team == 2)) {  // blauw pakt rode vlag
        printf_("blauw pakt rode vlag\n");
        id_has_redflag = lnk->id;
        send_flagcarrier(lnk->id, 1);

        // of de vlag ergens anders ligt:
      } else if (((red_x != red_def_x) && (red_y != red_def_y)) && (lnk->team == 1)) {
    ...
    if ((sqrt((lnk->x - blue_x) * (lnk->x - blue_x) + (lnk->y - blue_y) * (lnk->y - blue_y)) <= (BLOCKSIZE / 2))) {
      printf_("ok er staat iemand op de blauwe vlag");
      if ((id_has_blueflag == -1) && (lnk->team == 1)) {  // rood pakt blauwe vlag
        printf_("rood pakt blauwe vlag\n");
        id_has_blueflag = lnk->id;
        send_flagcarrier(lnk->id, 2);

      } else if (((blue_x != blue_def_x) && (blue_y != blue_def_y)) && (lnk->team == 2)) {
  ...
  if ((id_has_blueflag != -1) && (id_has_redflag == -1) &&
      ((sqrt((blueflagcarrier->x - red_def_x) * (blueflagcarrier->x - red_def_x) +
             (blueflagcarrier->y - red_def_y) * (blueflagcarrier->y - red_def_y)) <=
        (BLOCKSIZE / 2)))) {  // red captured blueflag
    id_has_blueflag = -1;
    red_team_score++;
  ...
  if ((id_has_redflag != -1) && (id_has_blueflag == -1) &&
      ((sqrt((redflagcarrier->x - blue_def_x) * (redflagcarrier->x - blue_def_x) +
             (redflagcarrier->y - blue_def_y) * (redflagcarrier->y - blue_def_y)) <=
        (BLOCKSIZE / 2)))) {  // blue captured redflag
    id_has_redflag = -1;
    blue_team_score++;
...
#define SERV_HOST_ADDR "127.0.0.1" /* Change this to be your host addr */
  ...
  if (setsockopt(listener, IPPROTO_TCP, TCP_NODELAY, (char *)&just_say_no, sizeof just_say_no) == -1) {
    perror("setsockopt");
    exit(1);
  }
      ...
      if (FD_ISSET(i, &read_fds)) {  // we got one!!
        if (i == listener) {
...
#endif
            /* make socket nonbuffering */
            if (setsockopt(newfd, IPPROTO_TCP, TCP_NODELAY, (char *)&just_say_no, sizeof just_say_no) == -1) {
              perror("setsockopt");
              exit(1);
            }
          ...
          if (!client) {
            ...
            readpacket(client, i);
            if (IsDead(client)) {
              if (client->enabled == STATE_DEADSEND) quit_player(client, client->errmsg);
