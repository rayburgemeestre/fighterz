...
void sendulist(struct data *client) {
  ...
  for (current = head; current; current = current->next) {
    send_newuser(client,
                 ...
                 current->frags,
                 current->team,
                 (signed char)current->turn,
                 (unsigned char)current->type,
                 current->speed,
                 current->shiptype,
                 current->nick);
    // send_newuser(
    if (current->invincible) {
      ...
      send_invincible(NULL, NULL, current->id, (unsigned char)current->invincible, current->invincibility_t - diff);
...
void printulist() {
  ...
  current = head;
  while (current != NULL) {
    if (current->bullet != 1) printul((char *)(current->nick), (long int)(current->frags));
