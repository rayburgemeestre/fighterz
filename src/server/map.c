...
void loadmap() {
  ...
  while (fgets(buff, 500, fileptr) != NULL) {
    if (buff[0] == '1') {
      ...
      memmove(buff, src, strlen(src) + 1);

      if (cnt == 0) X_BLOCKS = (int)strlen(buff);
...
void set_map_coords() {
  if ((servertime - map_scroll) >= 10) {
    ...
    if (key[KEY_LEFT]) MAP_X -= 10;
    if (key[KEY_RIGHT]) MAP_X += 10;
    if (key[KEY_UP]) MAP_Y -= 10;
    if (key[KEY_DOWN]) MAP_Y += 10;
  }

  if (MAP_X < 0) MAP_X = 0;
  if ((unsigned)MAP_X > ((field_width + 1) - MAP_W)) MAP_X = (field_width + 1) - MAP_W;

  if (MAP_Y < 0) MAP_Y = 0;
  if ((unsigned)MAP_Y > ((field_height + 1) - MAP_H)) MAP_Y = (field_height + 1) - MAP_H;
}
  ...
  */
  ...
  for (cnt = 0; cnt < X_BLOCKS; cnt++) {
    for (cnt2 = 0; cnt2 < Y_BLOCKS; cnt2++) {
      if (field[cnt2][cnt] == '1')
        rectfill(
            fieldbuff, (cnt * BLOCKSIZE), (cnt2 * BLOCKSIZE), ((cnt + 1) * BLOCKSIZE), ((cnt2 + 1) * BLOCKSIZE), blauw);
