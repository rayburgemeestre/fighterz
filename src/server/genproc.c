      ...
      if (current->init_lag > 1) {
        if ((servertime - (current->last_lag_check)) >= LAG_DELAY) {
          ...
          if (current->init_lag == 1) {
            ...
            send_blockinfo(current, X_BLOCKS, Y_BLOCKS, BLOCKSIZE);
            for (cnt = 0; cnt < 32; cnt++) {
              if (strlen(bg_imgs_data[cnt].datfile) > 0)
                send_background(current,
...
double current_lag(ID sockid) {
  ...
  for (i = 0; i < 5; i++) _current_lag += current->lag[i];
...
void drawgraphics() {
  ...
  if (show_fps == 1 && head != NULL) {
    textprintf(tmpscreen,
               font,
               1,
               1,
               15,
               "FPS: %d spd:%.2f lg:%2.2f bc:%du  %d,%d",
               fps,
               (head != NULL ? head->speed : 0.0),
               (head != NULL ? current_lag(head->id) : 0.0),
               head->bulletcnt,
               (int)REDFLAG.x,
               (int)REDFLAG.y);
...
void initialize_debug() {
  if (path_calc_dbg_on) path_calc_dbg = fopen("pathcalc.txt", "w+");
...
int valid_target(double x1, double y1, double x2, double y2, double precision) {
  ...
  while ((dabs(cur_x - x2) > precision || dabs(cur_y - y2) > precision)) {
    ...
    if (map[(int)(tmp_y / BLOCKSIZE)][(int)(tmp_x / BLOCKSIZE)] == 16384) collided = 1;

    if (map[(int)(lw_y / BLOCKSIZE)][(int)(lw_x / BLOCKSIZE)] == 16384) collided = 1;

    if (map[(int)(rw_y / BLOCKSIZE)][(int)(rw_x / BLOCKSIZE)] == 16384) collided = 1;
