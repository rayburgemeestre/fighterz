...
#define BOUNCE_POSSIBLE_OR_RESTORE                                             \
  ...
  if (!(px > X_BLOCKS) && !(px < 0) && !(py > Y_BLOCKS) && !(py < 0)) {
    /* center of the ship */
    // if (field[py][px] == 1)
    //	collided = true;
    ...
    if ((py + 1) <= Y_BLOCKS) {
      ...
      if (field[qy][px] == '1') {
        ...
        if (!nobounce && dabs(current->speed) > 0.05)  // static minimal speed required
        {
          ...
          if (current->deg >= (270 - deg_diff_max) && current->deg <= 270) {
            current->deg = 270 + ((270 - current->deg) / rdemp);
            BOUNCE_POSSIBLE_OR_RESTORE
          }
          ...
          else if (current->deg >= (90 - deg_diff_max) && current->deg <= 90) {
    ...
    if ((px - 1) >= 0) {
      ...
      if (field[py][qx] == '1') {                             // TODO; find out why :)
        ...
        if (!nobounce && dabs(current->speed) > 0.05)  // static minimal speed required
        {
          ...
          if (current->deg >= (360 - deg_diff_max) && current->deg <= 360) {
            current->deg = (360 - current->deg) / rdemp;
          ...
          else if (current->deg >= (180 - deg_diff_max) && current->deg <= (180 + deg_diff_max)) {
...
double futureX(struct data *ptr, double angle, double speed) {
  return _futureX(ptr->x, angle, speed);
}
double futureY(struct data *ptr, double angle, double speed) {
  return _futureY(ptr->y, angle, speed);
}
#endif
...
int collidecheck2b(struct data *ptr) {
  ...
  current = head;
  while (current != NULL) {
    if ((struct data *)current == ptr) break;
    current = current->next;
  }
...
#define MARGE (BLOCKSIZE / 8)
  ...
  if (collided == 1) {
    ...
    fflush(stdout);
#endif
    if (current->owner != NULL) current->owner->bulletcnt--;
    del_bullet(current);
...
int bs_collidecheck(struct data *ptr) {
  ...
  if (tmp != NULL) {
    if (tmp->bullet == 1) {
      ...
      while (tmp2 != NULL) {
        i = 0;
        if ((tmp2->bullet != 1) && (tmp->owner != tmp2) && (tmp->owner->bullet != 1) && (tmp2->dead == 0) &&
            (tmp2->invincible == 0)) {
          /* Compare coordinates with this ship */
