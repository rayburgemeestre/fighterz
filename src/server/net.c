...
void readpacket(struct data *client, int s) {
  ...
  client->inlen = len;
  if (len > 0) memcpy(client->buffer, p, (size_t)len);
  return;
}
...
#if 1 == 2
  int enabled = 1;
  if (fcntl(s, F_SETFL, O_NDELAY) < 0) return -1;
  if (setsockopt(s, IPPROTO_TCP, TCP_NODELAY, (char *)&enabled, sizeof(enabled)) < 0) return -1;
#endif
  return 1;
}
