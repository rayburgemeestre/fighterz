...
int isvalid(char c) {
  if (isalpha(c) || isdigit(c) || (c == '-') || (c == '_') || (c == '[') || (c == ']') || (c == '|') || (c == '^'))
    return 1;
  else
    return 0;
}
...
static void m_nick(struct data *client, char *newnick) {
  ...
  if (!IsRegistered(client)) {
    ...
    send_newuser(EVERYONE,
                 client,
                 client->id,
                 client->x,
                 client->y,
                 client->deg,
                 (signed char)client->velocity,
                 client->alive,
                 client->frags,
                 client->team,
                 (signed char)client->turn,
                 (unsigned char)client->type,
                 client->speed,
                 client->shiptype,
                 client->nick);
...
static void m_lag(struct data *client, unsigned int t) {
  ...
  for (i = 0; i < 4; i++) client->lag[i] = client->lag[i + 1];
  client->lag[4] = (double)diff;
}

static void m_spawn(struct data *client) {
  if (client->dead != 2) return; /* Not allowed to respawn (yet), ignore */
...
void m_newbullet(struct data *client, double x, double y, double deg) {
  ...
  if (client->bulletcnt == (unsigned)BULLET_MAX) {
    ...
    for (current2 = head; current2; current2 = current2->next)
      if (IsBullet(current2) && (current2->owner == client)) oldestbullet = current2;
    if (!oldestbullet) {
...
static void m_cmd(struct data *client, char *cmd) {
  ...
  cleanstr(cmd);
  len = strlen(cmd);
  if (len == 0) return; /* Ignore empty msgs */
  if (len > 512) cmd[511] = '\0';
  ...
  again:
    ...
    return;
  } else if (!strcmp(cmd, "ship1")) {
    client->shiptype = 1;
    send_newship(client);
  } else if (!strcmp(cmd, "ship2")) {
    client->shiptype = 2;
    send_newship(client);
  } else if (!strcmp(cmd, "ship3")) {
    client->shiptype = 3;
    send_newship(client);
  } else if (!strcmp(cmd, "ship4")) {
    client->shiptype = 4;
    send_newship(client);
  } else if (!strcmp(cmd, "ship5")) {
    client->shiptype = 5;
    send_newship(client);
  } else if (!strcmp(cmd, "ship6")) {
    client->shiptype = 6;
    send_newship(client);
  }
}
...
void dopacket(struct data *client, unsigned short xtype, unsigned short len, char *dta) {
  ...
  switch (xtype) {
    case CMSG_VERSION: {
      unsigned int ver;
      printf("got version\n");
      if (!get_u32(&ver, &dta, &len)) goto fatal;
    ...
    case CMSG_NICK: {
      char newnick[256];
      printf("got nick.. \n");
      if (!get_str(newnick, &dta, &len, sizeof(newnick))) goto fatal;
      printf("got nick %s\n", newnick);
    ...
    case CMSG_PONG: {
      unsigned int datime;
      if (!get_u32(&datime, &dta, &len)) goto fatal;
      m_lag(client, datime);
      return;
    }
    default:
  ...
  switch (xtype) {
    ...
    case CMSG_CMD: {
      char cmd[512];
      if (!get_str(cmd, &dta, &len, sizeof(cmd))) goto fatal;
    ...
    case CMSG_SAY: {
      char text[512];
      if (!get_str(text, &dta, &len, sizeof(text))) goto fatal;
