...
void init() {
  ...
  if (!dat_sound) alert("Could not load sound", "", "", "", NULL, 0, 0);
...
// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
struct data *add_player(int _id,
                        int _x,
                        int _y,
                        int _deg,
                        int _move,
...
double current_lag() {
  ...
  for (i = 0; i < 5; i++) current_lag += lag[i];
...
void draw_talk_box() {
  ...
  textprintf(bmp_command, (FONT *)dat_base[NOKIA].dat, 5, 5, makecol(255, 255, 255), "%s", msg);
}
...
#define FLAG_ANIM_ITERATOR 500
  ...
  if ((prev_flag_time == 0) || ((ourtime - prev_flag_time) >= FLAG_ANIM_ITERATOR)) {
...
void blit_field() {
  ...
  if (show_radar == 1) draw_sprite(bmp_radar, (BITMAP *)dat_base[GUI_RADAR_BG].dat, 0, 0);
}
...
int mainloop() {
  ...
  while (!key[KEY_F12] && !terminate_request) {
    ...
    if (!game_started && our_spawnstatus == 0) continue; /* Don't do graphics */
  ...
  // clean up
  while (head != NULL) del_player(head->id);
