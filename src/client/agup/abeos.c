...
/* Elias used some Allegro 4.1+ features, which we will emulate here
 * for people using Allegro 4.0. --pw
 */

#if (ALLEGRO_VERSION == 4) && (ALLEGRO_SUB_VERSION == 0)

static void textout_ex(BITMAP *bmp, FONT *font, AL_CONST char *s, int x, int y, int fg, int bg) {
  int rtm = text_mode(bg);
  textout(bmp, font, s, x, y, fg);
  text_mode(rtm);
}

static void gui_textout_ex(BITMAP *bmp, AL_CONST char *s, int x, int y, int fg, int bg, int centre) {
  int rtm = text_mode(bg);
  gui_textout(bmp, s, x, y, fg, centre);
  text_mode(rtm);
}
 ...
 *
 */
void beos_box(BITMAP *bmp, int x, int y, int w, int h, int flags, enum ABEOS_WIDGET_TYPE what, struct BDATA *bdata) {
  switch (what) {
    case ABEOS_PUSH_BUTTON: /* button */
    {
      ...
      if (flags & D_GOTFOCUS) c = highlight;
      if (flags & D_DISABLED) {
        c = disabled;
        tc = abeos_mg_color;
      }
      ...
      if (flags & D_SELECTED && flags & D_DISABLED) c = disabled_pushed;
      ...
      if (bdata) gui_textout_ex(bmp, (char *)bdata->u.text, x + tx, y + ty, tc, -1, center);
      break;
    }
    ...
    case ABEOS_RADIO_BUTTON: {
      ...
      if (what == ABEOS_RADIO_BUTTON) b = radio;

      if (flags & D_SELECTED) o |= 1;
      if (flags & D_GOTFOCUS) o |= 2;
    ...
    case ABEOS_FRAME: {
      ...
      if (flags & D_GOTFOCUS) c = frame_highlight;
      ...
      if (bdata) {
        ...
        if (total > visible) {
          ...
          vline(screen, x + w - 13, y + 2, y + h - 4, shadow_color);

          beos_box(screen, x + w - 12, y + 2, 9, h - 5, flags, ABEOS_SCROLLBAR_BACK, NULL);
          beos_box(screen, x + w - 12, y + 2 + off, 9, off2 - off + 1, flags, ABEOS_SCROLLBAR_HANDLE, NULL);

          if (off2 + 1 < h - 6) hline(screen, x + w - 12, y + 3 + off2, x + w - 4, shadow_color);
        }
      }
      break;
    }
    case ABEOS_SCROLLBAR_HANDLE: {
      ...
      if (flags & D_GOTFOCUS) c = scroll_hl;

      if (flags & D_DISABLED) c = scroll_dis;
    ...
    case ABEOS_SLIDER: {
      ...
      if (flags & D_DISABLED) c = slider_dis;
      ...
      if (bdata) {
        ...
        int total = bdata->u.scroll.total;
        int visible = bdata->u.scroll.visible;
        BITMAP *handle = flags & D_GOTFOCUS ? sliderhandle[1] : sliderhandle[0];
    ...
    case ABEOS_EDITBOX_FRAME: {
      int *c = frame2;
      if (flags & D_GOTFOCUS) c = frame2_highlight;
    ...
    case ABEOS_WINDOW_FRAME: {
      ...
      if (bdata) textout_ex(bmp, font, (char *)bdata->u.text, x + 6, y + 4, abeos_fg_color, -1);
...
static void plot(BITMAP *bmp, char data[13][14], struct datapal *pal) {
  int x, y, i;
  for (y = 0; y < bmp->h; y++) {
    for (x = 0; x < bmp->w; x++) {
      for (i = 0; pal[i].c; i++) {
...
int d_abeos_box_proc(int msg, DIALOG *d, int c) {
  (void)c;
  if (msg == MSG_DRAW) beos_box(screen, d->x, d->y, d->w, d->h, d->flags, ABEOS_PUSH_BUTTON, NULL);

  return D_O_K;
}

int d_abeos_shadow_box_proc(int msg, DIALOG *d, int c) {
  (void)c;
  if (msg == MSG_DRAW) beos_box(screen, d->x, d->y, d->w, d->h, d->flags, ABEOS_PUSH_BUTTON, NULL);
...
int d_abeos_button_proc(int msg, DIALOG *d, int c) {
  if (msg == MSG_DRAW) {
    struct BDATA bdata;
    bdata.u.text = (char *)d->dp;
    beos_box(screen, d->x, d->y, d->w, d->h, d->flags, ABEOS_PUSH_BUTTON, &bdata);
    return D_O_K;
  }
  return d_button_proc(msg, d, c);
}

int d_abeos_push_proc(int msg, DIALOG *d, int c) {
  int ret = D_O_K;
  d->flags |= D_EXIT;
...
int d_abeos_check_proc(int msg, DIALOG *d, int c) {
  if (msg == MSG_DRAW) {
    struct BDATA bdata;
    bdata.u.text = (char *)d->dp;
    beos_box(screen, d->x, d->y, d->w, d->h, d->flags, ABEOS_CHECK_BUTTON, &bdata);
    return D_O_K;
  }
  return d_button_proc(msg, d, c);
}

int d_abeos_radio_proc(int msg, DIALOG *d, int c) {
  if (msg == MSG_DRAW) {
    struct BDATA bdata;
    bdata.u.text = (char *)d->dp;
    beos_box(screen, d->x, d->y, d->w, d->h, d->flags, ABEOS_RADIO_BUTTON, &bdata);
    return D_O_K;
  }
  return d_radio_proc(msg, d, c);
}

int d_abeos_icon_proc(int msg, DIALOG *d, int c) {
  if (msg == MSG_DRAW) {
    beos_box(screen, d->x, d->y, d->w, d->h, d->flags, ABEOS_PUSH_BUTTON, NULL);
    stretch_sprite(screen, (BITMAP *)d->dp, d->x + 2, d->y + 2, d->w - 4, d->h - 4);
    return D_O_K;
  }
  return d_abeos_button_proc(msg, d, c);
}

int d_abeos_edit_proc(int msg, DIALOG *d, int c) {
  ...
  if (msg == MSG_DRAW) {
    ...
    rectfill(screen, d->x + 2, d->y + 2, d->x + d->w - 3, d->y + text_height(font) + 3, white);
    ...
    for (x = 4; p <= b; p++) {
      ...
      textout_ex(screen, font, buf, d->x + x, d->y + 4, fg, -1);

      if (f && abeos_time_toggle) vline(screen, d->x + x - 1, d->y + 3, d->y + text_height(font) + 3, abeos_fg_color);
      if ((x += w) + w > d->w - 4) break;
    }

    beos_box(screen, d->x, d->y, d->w, text_height(font) + 8, d->flags, ABEOS_EDITBOX_FRAME, NULL);

    return D_O_K;
  }

  if (msg == MSG_LOSTFOCUS || msg == MSG_LOSTMOUSE) return D_WANTFOCUS;

  return d_edit_proc(msg, d, c);
}

int d_abeos_list_proc(int msg, DIALOG *d, int c) {
  if (msg == MSG_DRAW) {
    ...
    for (i = 0; i < height; i++) {
      if (d->d2 + i < size) {
        ...
        ustrzcpy(s, sizeof(s), (*(char *(*)(int, int *))d->dp)(i + d->d2, NULL));
        x = d->x + 2;
        ...
        if (x <= d->x + 1 + w) rectfill(screen, x, y, d->x + 1 + w, y + text_height(font) - 1, bg);
      } else {
        rectfill(screen,
                 d->x + 2,
                 d->y + 2 + i * text_height(font),
                 d->x + 1 + w,
                 d->y + 1 + (i + 1) * text_height(font),
                 normal[0]);
      }
    }

    if (d->y + 2 + i * text_height(font) <= d->y + d->h - 3)
      rectfill(screen, d->x + 2, d->y + 2 + i * text_height(font), d->x + 1 + w, d->y + d->h - 4, normal[0]);
    ...
    /* draw the frame around */
    {
      ...
      beos_box(screen, d->x, d->y, d->w, d->h, d->flags, ABEOS_FRAME, &bdata);
    }
...
int d_abeos_text_list_proc(int msg, DIALOG *d, int c) {
  if (msg == MSG_DRAW) return d_abeos_list_proc(msg, d, c);

  return d_text_list_proc(msg, d, c);
}

int d_abeos_textbox_proc(int msg, DIALOG *d, int c) {
  int height, bar = 1;
...
int d_abeos_slider_proc(int msg, DIALOG *d, int c) {
  if (msg == MSG_DRAW) {
    struct BDATA bdata;
    bdata.u.scroll.total = d->d1;
    bdata.u.scroll.pos = d->d2;
    bdata.u.scroll.visible = 1;
    beos_box(screen, d->x, d->y, d->w, d->h, d->flags, ABEOS_SLIDER, &bdata);
    return D_O_K;
  }
...
static void abeos_draw_menu(int x, int y, int w, int h) {
...
static void abeos_draw_menu_item(MENU *m, int x, int y, int w, int h, int bar, int sel) {
  int fg, bg;
  int i, j;
  char buf[256], *tok;
  ...
  if (!menu_done) {
    if (bar)
      beos_box(screen, last_x, last_y, last_w, last_h, 0, ABEOS_MENUBAR, NULL);
    else
      beos_box(screen, last_x, last_y, last_w, last_h, 0, ABEOS_MENU_FRAME, NULL);
    menu_done = TRUE;
  }
  ...
  if (ugetc(m->text)) {
    ...
    if ((m->child) && (!bar)) draw_sprite(screen, arrow[sel ? 1 : 0], x + w - 12, y + h / 2 - 7);
  } else {
    /* menu separator */
    hline(screen, x, y + text_height(font) / 2 - 1, x + w - 1, menu[4]);
    hline(screen, x, y + text_height(font) / 2, x + w - 1, menu[1]);
  }
...
int d_abeos_menu_proc(int msg, DIALOG *d, int c) {
...
int d_abeos_window_proc(int msg, DIALOG *d, int c) {
  (void)c;

  if (msg == MSG_DRAW) {
    struct BDATA bdata;
    bdata.u.text = (char *)d->dp;
    beos_box(screen, d->x, d->y, d->w, d->h, d->flags, ABEOS_WINDOW_FRAME, d->dp ? &bdata : NULL);
  }
...
char checkoverlay2[13][14] = {"xxxxxxxxxxxxx",
                              "x           x",
                              "x           x",
                              "x           x",
                              "x           x",
...
char radiooverlay2[13][14] = {"    xxxxx    ",
                              "  xx     xx  ",
                              " x         x ",
                              " x         x ",
                              "x           x",
...
struct datapal pal_hl[] = {{'/', 235, 235, 235},
                           {'.', 184, 184, 184},
                           {'x', 175, 123, 0},
                           {'|', 250, 250, 80},
                           {'_', 255, 203, 0},
...
struct datapal pal_dis[] = {{'/', 235, 235, 235},
                            {'.', 216, 216, 216},
                            {'x', 152, 152, 152},
                            {'|', 239, 239, 239},
                            {'_', 184, 184, 184},
...
static struct agup_theme the_theme = {&abeos_fg_color,
                                      &abeos_bg_color,
                                      abeos_init,
                                      abeos_shutdown,
                                      d_abeos_box_proc,
