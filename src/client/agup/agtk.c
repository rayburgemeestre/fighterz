...
int d_agtk_box_proc(int msg, DIALOG *d, int c) {
  (void)c;
  if (msg == MSG_DRAW) gtk_box(screen, d->x, d->y, d->w, d->h, 0, 0);
  return D_O_K;
}

int d_agtk_shadow_box_proc(int msg, DIALOG *d, int c) {
  (void)c;
  if (msg == MSG_DRAW) gtk_box(screen, d->x, d->y, d->w, d->h, 0, 1);
  return D_O_K;
}
  ...
  if (msg == MSG_DRAW) {
    ...
    if (d->dp) {
      ...
      gui_textout(screen,
                  ...
                  (d->flags & D_DISABLED) ? nshadow : black,
                  TRUE);
      text_mode(rtm);
    }
    return D_O_K;
...
int d_agtk_text_list_proc(int msg, DIALOG *d, int c) {
  if (msg == MSG_DRAW) return d_agtk_list_proc(msg, d, c);
...
static struct agup_theme the_theme = {&agtk_fg_color,
                                      &agtk_bg_color,
                                      agtk_init,
                                      agtk_shutdown,
                                      d_agtk_box_proc,
