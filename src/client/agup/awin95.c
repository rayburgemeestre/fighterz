  ...
  for (y = 0; y < h; y++)
    for (x = 0; x < w; x++) switch (templat[y][x]) {
        ...
        case 'X':
          ...
          break;
        case 'x':
          putpixel(bmp, x, y, nshadow);
          break;
        case 'a':
          putpixel(bmp, x, y, hshadow);
          break;
        case ' ':
...
static INLINE void dotted_rect(int x1, int y1, int x2, int y2, int fg, int bg) {
  ...
  for (c = x1; c <= x2; c++) putpixel(screen, c, y1, (((c + y1) & 1) == x) ? fg : bg);
  for (c = x1; c <= x2; c++) putpixel(screen, c, y2, (((c + y2) & 1) == x) ? fg : bg);
...
int d_awin95_box_proc(int msg, DIALOG *d, int c) {
  (void)c;
  if (msg == MSG_DRAW) win95_box(screen, d->x, d->y, d->w, d->h, 0);
  return D_O_K;
}
  ...
  if (msg == MSG_DRAW) {
    ...
    if (d->dp) {
      ...
      gui_textout(screen,
                  ...
                  (d->flags & D_DISABLED) ? gray : black,
                  TRUE);
      text_mode(rtm);
    }

    if (d->flags & D_GOTFOCUS) dotted_rect(d->x + 4, d->y + 4, d->x + d->w - 5, d->y + d->h - 5, black, normal);
...
int d_awin95_check_proc(int msg, DIALOG *d, int c) {
  if (msg == MSG_LRELEASE || msg == MSG_LPRESS) d->flags |= D_DIRTY;

  if (msg == MSG_DRAW) {
    int x = 0;
    if ((d->flags & D_GOTMOUSE) && (mouse_b & 1)) x |= 4;
    if (d->flags & D_SELECTED) x |= 2;
    if (d->flags & D_GOTFOCUS) x |= 1;

    draw_base(screen, d);
    win95_check_bevel(screen, d->x + 2, d->y + d->h / 2 - 6, 13, 13, x);

    /* Draw X */
    if (x & 2) draw_sprite(screen, checked_bmp, d->x + 4, d->y + d->h / 2 - 4);
    ...
    if (x & 1)
      dotted_rect_wh(d->x + 16,
                     d->y + d->h / 2 - text_height(font) / 2 - 2,
                     text_length(font, (char *)d->dp) + 4,
                     text_height(font) + 4,
                     black,
                     normal);
...
int d_awin95_radio_proc(int msg, DIALOG *d, int c) {
  ...
  if (msg == MSG_DRAW) {
    ...
    draw_sprite(screen,
                (d->flags & D_SELECTED)
                    ? (((d->flags & D_GOTMOUSE) && (mouse_b & 1)) ? radio_pressed_bmp : radio_down_bmp)
                    : radio_up_bmp,
                d->x + 3,
                d->y + d->h / 2 - 6);
...
int d_awin95_edit_proc(int msg, DIALOG *d, int c) {
  ...
  if (msg == MSG_DRAW) {
    ...
    for (x = 4; p <= b; p++) {
      ...
      text_mode(rtm);
      if (f && awin95_time_toggle) vline(screen, d->x + x - 1, d->y + 3, d->y + text_height(font) + 3, black);
...
int d_awin95_list_proc(int msg, DIALOG *d, int c) {
  if (msg == MSG_DRAW) {
    ...
    if (d->y + 2 + i * text_height(font) <= d->y + d->h - 3)
      rectfill(screen, d->x + 2, d->y + 2 + i * text_height(font), d->x + w, d->y + d->h - 3, white);
...
int d_awin95_text_list_proc(int msg, DIALOG *d, int c) {
  if (msg == MSG_DRAW) return d_awin95_list_proc(msg, d, c);
...
static void win95_draw_menu_item(MENU *m, int x, int y, int w, int h, int bar, int sel) {
  ...
  if (ugetc(m->text)) {
    ...
    if ((m->child) && (!bar)) draw_sprite(screen, menu_arrow_bmp, x + w - 12, y + (h - menu_arrow_bmp->h) / 2);
  } else {
...
static void fill_textout(BITMAP *bmp, FONT *f, AL_CONST char *text, int x, int y, int w, int fg, int bg) {
  int text_w, text_h;
  int old_text_mode;
  int cl, ct, cr, cb;
  ...
  if (is_screen_bitmap(bmp)) scare_mouse_area(x, y, x + w - 1, y + text_h - 1);

  cl = bmp->cl, ct = bmp->ct, cr = bmp->cr, cb = bmp->cb;
  set_clip(bmp, x, y, x + w - 1, y + text_h - 1);

  textout(bmp, f, text, x, y, fg);
  if (text_w < w) rectfill(bmp, x + text_w, y, x + w - 1, y + text_h - 1, bg);

  set_clip(bmp, cl, ct, cr, cb);

  if (is_screen_bitmap(bmp)) unscare_mouse();
...
static void draw_window_title(DIALOG *d, AL_CONST char *text) {
  ...
  fill_textout(screen,
               ...
               white,
               blue);
  hline(screen, d->x + border_thickness, d->y + border_thickness, d->x + d->w - border_thickness - 2, nshadow);
  vline(screen,
...
static struct agup_theme the_theme = {&awin95_fg_color,
                                      &awin95_bg_color,
                                      awin95_init,
                                      awin95_shutdown,
                                      d_awin95_box_proc,
