  ...
  for (current = head; current != NULL; current = current->next) {
    ...
    if ((current->bullet != 1) && (current->dead == 0)) {
      ...
      if (current->bot == 1) {
        ...
        if ((!mod_grid) && (current->shiptype != 6)) {  // shiptype 6 = old school ship
          int shiptype;
          ...
          {  // fill polygon with black
            int points[8] = {pos_x, pos_y, lw_x, lw_y, current_x, current_y, rw_x, rw_y};
            int *p = points;
      ...
      if (show_names == 1) {
        ...
        textprintf_centre(bmp_shipfield,
                          (FONT *)dat_base[NOKIA].dat,
                          current_x,
                          current_y + (blocksize / 2) + 3,
                          teamcolor,
                          "%s",
                          tmpstr);
      }
...
void move_ships() {
  for (current = head; current != NULL; current = current->next) {
    ...
    if (current->bullet != 1) {
      ...
      if ((ourtime - current->hit_t) >= 200) current->impact = 0;
    }
  }
}
...
void move_ship(struct data *client) {
  ...
  for (cnt = 0; cnt < times; cnt++) {
    ...
    {
      if (client->speed < client->max_speed) client->speed += 0.01;
    } else if (client->velocity == 0.00)  // speed must become zero
    {
      if (client->speed < -0.001) {  // we don't want -0.00 to valid this statement
        client->speed += 0.01;
        if (client->speed == 0.00) client->speed = 0.00;
    ...
    {
      if (client->speed > (-1 * client->max_speed)) client->speed -= 0.01;
    } else
  ...
  for (cnt = 0; cnt < times; cnt++) {
    ...
    if (ret == 1) break;  // dont move, because the next move would collide
...
void draw_bullets() {
  ...
  for (current = head; current != NULL; current = current->next) {
    if (current->bullet == 1) {
      circlefill(bmp_shipfield, current->x - x_on_map, current->y - y_on_map, 1, makecol(255, 255, 0));
    }
  }
}
...
void del_bullet(struct data *ptr) {
  ...
  for (current = head; current; current = current->next) {
    ...
    if ((struct data *)current == ptr) {
      if (current->owner == our_node) our_bullet_count--;
...
int move_bullet(struct data *bullet, unsigned long t2) {
  ...
  for (cnt = 0; cnt < times; cnt++) {
    ...
    if (collidecheck2b(bullet) == 1) return 1;  // so we return 1 as well, because this bullet is dead.
    ...
    if (bs_collidecheck(bullet) == 1) return 1;  // this bullet is now dead
  }
...
void draw_explosions() {
  ...
  for (current = expl_head; current;) {
    ...
    for (cnt = 0; cnt < times; cnt++) current->radius++;
...
void del_explosion(struct data2 *ptr) {
  ...
  while (current != NULL) {
    future = current->next;
    if (current == ptr) {
      if (current == expl_head) { /* first node */
        future = current->next;
        free(current);
        expl_head = future;
        return;
