...

  {
    ...
    for (i = 0; i < 32; i++) {
      if (strlen(bg_imgs_data[i].datfile) > 0) {
        if (!mod_grid) plot_datimg(bg_imgs_data[i]);
      }
    }
  }
  ...
  // DRAW RADARBUFF
  {
    ...
    for (cnt = 0; cnt < map_blocks_x; cnt++) {
      for (cnt2 = 0; cnt2 < map_blocks_y; cnt2++) {
        if (field[cnt2][cnt] == '1') {
          rectfill(
              bmp_radarfield, cnt * scale, cnt2 * scale, (cnt + 1) * scale, (cnt2 + 1) * scale, makecol(53, 18, 22));
        }
      }
    }
  }
...
void draw_radar() {
  ...
  if (show_radar != 1) {
    ...
    // flickering
    if ((prev_radar_flickertime == 0) || ((ourtime - prev_radar_flickertime) >= 100)) {
      prev_radar_flickertime = ourtime;
      on = on == 1 ? 2 : 1;
    }
    // scanner
    while ((prev_radar_scannertime == 0) || ((ourtime - prev_radar_scannertime) >= 10)) {
    ...
    blit(a_redbar,
         ...
         radar_powerbar_y,
         radar_bar_w,
         radar_bar_h * ((100.0 - _power) / 100.0));
    ...
    blit(a_redbar,
         ...
         radar_bulletbar_y,
         radar_bar_w,
         radar_bar_h * ((100.0 - _bullets) / 100.0));

    {
      ...
      while (tmp) {
        ...
        if (tmp->bullet != 1) {
          if (tmp->bot == 1 && getdistance(tmp->x, tmp->y, our_node->x, our_node->y) > (RADAR_SENSOR / 2) * blocksize) {
            ...
            // show radar range
            radius = ((RADAR_SENSOR / 2) * blocksize) /
                     (blocksize /
                      scale);  // RADAR_SENSOR(50) blocks in diameter = half: (25 * blocksize) / (blocksize / scale)
...
void set_map_coords() {
  ...
  if (((our_node == NULL) && (game_started == 1)) || ((our_node != NULL) && (our_node->dead == 2))) {
    if ((ourtime - map_scroll) >= 10) {
      map_scroll = ourtime;

      if (key[KEY_LEFT]) x_on_map -= 10;
      if (key[KEY_RIGHT]) x_on_map += 10;
      if (key[KEY_UP]) y_on_map -= 10;
      if (key[KEY_DOWN]) y_on_map += 10;
    }
  } else {
  ...
  if (x_on_map < 0 || field_width <= (unsigned)field_w) x_on_map = 0;
  if ((unsigned)x_on_map > ((field_width + 1) - field_w)) x_on_map = (field_width + 1) - field_w;

  if (y_on_map < 0 || field_height <= (unsigned)field_h) y_on_map = 0;
  if ((unsigned)y_on_map > ((field_height + 1) - field_h)) y_on_map = (field_height + 1) - field_h;
}
