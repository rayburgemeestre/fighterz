...
int init_map(int allow_cancel) {
...

  {
    ...
    if (!(fi = fopen(buffer, "r"))) die("Error opening file");

    while (fgets(line, 512 - 1, fi) != NULL) {
      char *ptr = line;
      if (*ptr == '1') {
        if (strlen(line) <= 2) die("Invalid line input while reading mapfile");
        strcpy(mapdata[cnt], (ptr + 2));
        if ((signed)strlen(ptr + 2) > x_width) x_width = strlen(ptr + 2);
        y_width++;
        cnt++;
      } else if (*ptr == '3') {
    ...
    if (x_width <= 0 || y_width <= 0) die("both x_width & y_width must exceed zero");
  ...
  {
    char tmp[512];
    sprintf(tmp, "params %d, %d", (x_width * BLOCKSIZE) + 1, (y_width * BLOCKSIZE) + 1);
    // alert(tmp, "", "", "", NULL, 1, 1);
  }
...
void drawmap() {
  ...
  if (scrn_field) destroy_bitmap(scrn_field);
  scrn_field = create_bitmap((x_width * BLOCKSIZE) + 1, (y_width * BLOCKSIZE) + 1);
  {
    int x = SCREEN_X - RIGHT, y = SCREEN_Y - BOTTOM;
    if ((x_width * BLOCKSIZE) + 1 < x) x = (x_width * BLOCKSIZE) + 1;
    if ((y_width * BLOCKSIZE) + 1 < y) y = (y_width * BLOCKSIZE) + 1;
  ...
  for (cnt = 0; cnt < y_width; cnt++) {
    line(scrn_field, 0, (cnt * BLOCKSIZE), (x_width * BLOCKSIZE), (cnt * BLOCKSIZE), makecol(128, 128, 128));
  }
  line(scrn_field, 0, (cnt * BLOCKSIZE), (x_width * BLOCKSIZE), (cnt * BLOCKSIZE), makecol(128, 128, 128));
  for (cnt = 0; cnt < x_width; cnt++) {
    line(scrn_field, (cnt * BLOCKSIZE), 0, (cnt * BLOCKSIZE), (y_width * BLOCKSIZE), makecol(128, 128, 128));
  }
  line(scrn_field, (cnt * BLOCKSIZE), 0, (cnt * BLOCKSIZE), (y_width * BLOCKSIZE), makecol(128, 128, 128));
...
char *bmp_images(int index, int *list_size) {
  ...
  int item_cnt = 0;
  while (i < 32) {
    if (strlen(bmp_df[i])) item_cnt++;
    i++;
  }

  if (index < 0) {
    if (list_size) *list_size = item_cnt;
    return NULL;
  }
...
char *current_bmp_images(int index, int *list_size) {
  ...
  int item_cnt = 0;
  while (i < 32) {
    if (strlen(current_bmp_df[i])) item_cnt++;
    i++;
  }
