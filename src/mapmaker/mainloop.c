...
void mainloop() {
  ...
  while (!key[KEY_F12]) {
    ...
    ftime(&t);
    ourtime = (((t.time - starttime.time) * 1000) + ((t.millitm - starttime.millitm) / 1));

    // keep track of offsets
    diff = ((x_width * BLOCKSIZE) + 1) - (SCREEN_X - RIGHT);
    if (x_offset > diff) x_offset = (diff > 0 ? diff : 0);
    diff = ((y_width * BLOCKSIZE) + 1) - (SCREEN_Y - BOTTOM);
    if (y_offset > diff) y_offset = (diff > 0 ? diff : 0);
...
#define ZOOM_DELAY_MOVEPIXELS 1
    ...
    poll_mouse();
    if (mouse_x >= 0 && mouse_x <= (SCREEN_X - RIGHT) && mouse_y >= 0 && mouse_y <= (SCREEN_Y - BOTTOM) &&
        x <= x_width && y <= y_width) {
      if (mouse_b & 1) {
        if (the_dialog[DLG_NORMAL_WALL_CHECK].flags & D_SELECTED) {
          mapdata[y][x] = '1';
          if (mouse_x <= (x_width * BLOCKSIZE) && mouse_y <= (y_width * BLOCKSIZE))
          ...
          for (cnt = 0; cnt < x_width; cnt++) {
            for (cnt2 = 0; cnt2 < y_width; cnt2++)
              if (mapdata[cnt2][cnt] == '2') mapdata[cnt2][cnt] = '0';
          }
          // The new location
          mapdata[y][x] = '2';
          redraw_map = 1;
          ...
          for (cnt = 0; cnt < x_width; cnt++) {
            for (cnt2 = 0; cnt2 < y_width; cnt2++)
              if (mapdata[cnt2][cnt] == '3') mapdata[cnt2][cnt] = '0';
          }
          // The new location
          mapdata[y][x] = '3';
          redraw_map = 1;
          ...
          for (cnt = 0; cnt < x_width; cnt++) {
            for (cnt2 = 0; cnt2 < y_width; cnt2++)
              if (mapdata[cnt2][cnt] == '4') mapdata[cnt2][cnt] = '0';
          }
          // The new location
          mapdata[y][x] = '4';
          redraw_map = 1;
          ...
          for (cnt = 0; cnt < x_width; cnt++) {
            for (cnt2 = 0; cnt2 < y_width; cnt2++)
              if (mapdata[cnt2][cnt] == '5') mapdata[cnt2][cnt] = '0';
          }
          // The new location
          mapdata[y][x] = '5';
          redraw_map = 1;
          ...
          int w = ((BITMAP *)df[atoi(bmp_df[the_dialog[DLG_BG_IMG_LIST].d1])].dat)->w,
              ...
              x = x_offset + (mouse_x - (w / 2)), y = y_offset + (mouse_y - (h / 2));

          while (*(ptr + i) != '\0') {
            if (*(ptr + i) == '\\') ptr2 = ptr + i + 1;
            i++;
          }
          i = 0;
          ...
          if (mouse_x <= (SCREEN_X - RIGHT) && mouse_y <= (SCREEN_Y - BOTTOM)) {
            sprintf(buf, "%d (%d,%d) %s", atoi(bmp_df[the_dialog[DLG_BG_IMG_LIST].d1]), x, y, ptr2);
    ...
    while (((ourtime - last_zoom_time) / ZOOM_DELAY_MILLISECS) >= 1) {
      ...
      if (key[KEY_LSHIFT]) {
        ...
        if (key[KEY_RIGHT] && x_offset < ((x_width * BLOCKSIZE) - (SCREEN_X - RIGHT - 1))) {
          x_offset += ZOOM_DELAY_MOVEPIXELS;
        }
        if (key[KEY_UP] && y_offset > 0) {
          y_offset -= ZOOM_DELAY_MOVEPIXELS;
        }
        if (key[KEY_DOWN] && y_offset < ((y_width * BLOCKSIZE) - (SCREEN_Y - BOTTOM - 1))) {
          y_offset += ZOOM_DELAY_MOVEPIXELS;
        }
      }
    }
...
#define SIZE_Y 100
      ...
      blit(tussen, screen, 0, 0, SCREEN_X - RIGHT + 1, 285, SIZE_X, SIZE_Y);
      destroy_bitmap(tussen);
    }

    blit(scrn_field, sub_field, x_offset, y_offset, 0, 0, SCREEN_X - RIGHT, SCREEN_Y - BOTTOM);

    if (the_dialog[DLG_BG_IMG_CHECK].flags & D_SELECTED) {
      int w = ((BITMAP *)df[atoi(bmp_df[the_dialog[DLG_BG_IMG_LIST].d1])].dat)->w,
          h = ((BITMAP *)df[atoi(bmp_df[the_dialog[DLG_BG_IMG_LIST].d1])].dat)->h, x = mouse_x - (w / 2),
          y = mouse_y - (h / 2), x2 = x + w, y2 = y + h;

      // void rect(BITMAP *bmp, int x1, int y1, int x2, int y2, int color);
      rect(sub_field, x, y, x2, y2, makecol(255, 0, 0));
      // circlefill(sub_field, mouse_x, mouse_y, 50, makecol(255, 0, 0));
    }
    textprintf(screen, font, SCREEN_X - RIGHT + 3, SCREEN_Y - 31, makecol(255, 255, 255), "Pos. in blocks:");
    textprintf(screen, font, SCREEN_X - RIGHT + 3, SCREEN_Y - 21, 1023, "x_width:%3d", x);
    textprintf(screen, font, SCREEN_X - RIGHT + 3, SCREEN_Y - 11, 1023, "y_width:%3d", y);

    show_mouse(screen);
    if (!key[KEY_LSHIFT]) {
      int ret = update_dialog(player);

      if (!ret && player->obj == DLG_MAP_SPECS_OK_BUTTON) {
        int new_x = atoi(edit_x_width), new_y = atoi(edit_y_width);
        int cnt, cnt2;
        ...
        for (cnt = 0; cnt < 512; cnt++)  // y
        {
          ...
          {
            if (cnt >= new_x || cnt2 >= new_y) {
              mapdata[cnt2][cnt] = '\0';
            } else if (cnt >= x_width || cnt2 >= y_width) {
              mapdata[cnt2][cnt] = '0';
            }
          }
        }
        ...
        while (strlen(current_bmp_df[cnt])) cnt++;
